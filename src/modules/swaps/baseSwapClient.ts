...
import { baseSwapAbi } from "../../abi/baseSwapAbi"
import moment from "moment"
import { Hex } from "viem"

export class BaseSwapClient {
    walletClient: WalletClient;
    logger: any;
    loggerName: string = "BaseSwapClient";
    ...
    private async getAmountOut(from: token, to: token, amount: bigint) {
        const baseContract = new ethers.Contract(this.BaseSwapContract, baseSwapAbi, this.walletClient.walletEthers);
        const minAmountsOut = await baseContract.getAmountsOut(amount.toString(), [from.address[this.walletClient.chain.name], to.address[this.walletClient.chain.name]]);
    ...
    private async swapETHtoToken(from: token, to: token, amount: bigint): Promise<ethers.TransactionRequest> {
        ...
        const deadline = moment.now() + 1000000;

        const baseContract = new ethers.Contract(this.BaseSwapContract, baseSwapAbi, this.walletClient.walletEthers);
        const swapExactETHForTokens = await baseContract.swapExactETHForTokens.populateTransaction(minAmountOut, [from.address[this.walletClient.chain.name], to.address[this.walletClient.chain.name]], this.walletClient.walletAddress, deadline);
    ...
    private async swapTokenToETH(from: token, to: token, amount: bigint): Promise<ethers.TransactionRequest> {
        ...
        await this.walletClient.approve(this.BaseSwapContract, from, amount);

        const minAmountOut = await this.getAmountOut(from, to, amount);
        const deadline = moment.now() + 1000000;

        const baseContract = new ethers.Contract(this.BaseSwapContract, baseSwapAbi, this.walletClient.walletEthers);
        const swapExactETHForTokens = await baseContract.swapExactTokensForETH.populateTransaction(amount, minAmountOut, [from.address[this.walletClient.chain.name], to.address[this.walletClient.chain.name]], this.walletClient.walletAddress, deadline);
        const tx = await this.walletClient.prepareTransaction();
