...
import { alienSwapAbi } from "../../abi/alienSwapAbi"

export class AlienSwapClient {
    walletClient: WalletClient;
    logger: any;
    loggerName: string = "AlienSwapClient";

    AlienSwapContract: Hex = '0x8c1a3cf8f83074169fe5d7ad50b978e1cd6b37c7';
    ...
    private async getAmountOut(from: token, to: token, amount: bigint) {
        const alienContract = new ethers.Contract(this.AlienSwapContract, alienSwapAbi, this.walletClient.walletEthers);
        const minAmountsOut = await alienContract.getAmountsOut(amount.toString(), [from.address[this.walletClient.chain.name], to.address[this.walletClient.chain.name]]);
        return minAmountsOut[1] - (minAmountsOut[1] / 100n * BigInt(swapConfig.slippage));
    }
    ...
    private async swapETHtoToken(from: token, to: token, amount: bigint): Promise<ethers.TransactionRequest> {
        ...
        const alienContract = new ethers.Contract(this.AlienSwapContract, alienSwapAbi, this.walletClient.walletEthers);
        const swapExactETHForTokens = await alienContract.swapExactETHForTokens.populateTransaction(minAmountOut, [from.address[this.walletClient.chain.name], to.address[this.walletClient.chain.name]], this.walletClient.walletAddress, deadline);
    ...
    private async swapTokenToETH(from: token, to: token, amount: bigint): Promise<ethers.TransactionRequest> {
        ...
        await this.walletClient.approve(this.AlienSwapContract, from, amount);

        const minAmountOut = await this.getAmountOut(from, to, amount);
        const deadline = Math.floor(Date.now() / 1000) + 1000000;

        const alienContract = new ethers.Contract(this.AlienSwapContract, alienSwapAbi, this.walletClient.walletEthers);
        const swapExactETHForTokens = await alienContract.swapExactTokensForETH.populateTransaction(amount, minAmountOut, [from.address[this.walletClient.chain.name], to.address[this.walletClient.chain.name]], this.walletClient.walletAddress, deadline);
        const tx = await this.walletClient.prepareTransaction();
